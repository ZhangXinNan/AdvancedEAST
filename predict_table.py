...
def predict(east_detect, img, pixel_threshold, img_size=736):
    # d_wight, d_height = resize_image(img, cfg.max_predict_img_size)
    d_wight, d_height = resize_image(img, img_size)
    img = img.resize((d_wight, d_height), Image.NEAREST).convert('RGB')
    print('img resize : ', img.size, img.mode)  # img resize :  (640, 736) RGB
    img = image.img_to_array(img)
    img = preprocess_input(img, mode='tf')
    x = np.expand_dims(img, axis=0)
    print('x : ', x.shape)                      # x :  (1, 736, 640, 3)

    y = east_detect.predict(x)
    print("y : ", y.shape)                      # y :  (1, 184, 160, 7)
    y = np.squeeze(y, axis=0)
    print("y squeeze : ", y.shape)              # y squeeze :  (184, 160, 7)
    y[:, :, :3] = sigmoid(y[:, :, :3])
    # 1位score map, 是否在文本框内
    cond = np.greater_equal(y[:, :, 0], pixel_threshold)
    activation_pixels = np.where(cond)
    print('activation_pixels : ', len(activation_pixels), activation_pixels[0].shape, activation_pixels[1].shape)  #
    quad_scores, quad_after_nms = nms(y, activation_pixels)     # nms 实现
    print('quad_scores : ', quad_scores.shape)          # quad_scores :  (163, 4)
    print(' quad_after_nms :', quad_after_nms.shape)    # quad_after_nms : (163, 4, 2)
    return y, activation_pixels,quad_scores, quad_after_nms


def draw_quad(im, quad_scores, quad_after_nms, save_file, img_size=736, quiet=False):
    ...
    quad_im.save(save_file)
    # if cfg.predict_write2txt and len(txt_items) > 0:
    #     with open(img_path[:-4] + '_' + str(img_size) + '.txt', 'w') as f_txt:
    #         f_txt.writelines(txt_items)


def draw_act(im, y, activation_pixels, save_file, img_size=736):
    im_array = image.img_to_array(im.convert('RGB'))
    # d_wight, d_height = resize_image(im, cfg.max_predict_img_size)
    d_wight, d_height = resize_image(im, img_size)
    im = im.resize((d_wight, d_height), Image.NEAREST)  # .convert('RGB')
    print('cfg.pixel_size :', cfg.pixel_size)           # 4
    print('cfg.side_vertex_pixel_threshold :', cfg.side_vertex_pixel_threshold)     # 0.9
    print('cfg.trunc_threshold :', cfg.trunc_threshold)                             # 0.1
...
def main(args):
    img_filename = os.path.basename(args.path)
    print(args.path, args.threshold)

    img = image.load_img(args.path)
    print('img', img.size, img.mode)  # img (1782, 2019) RGB
    ...
    y, activation_pixels, quad_scores, quad_after_nms = predict(east_detect, img, args.threshold, img_size=args.img_size)

    img_quad_file = os.path.join(args.out_dir, img_filename + '_predict_zxtable_%d.jpg' % args.img_size)
    img_act_file = os.path.join(args.out_dir, img_filename + '_act_zxtable_%d.jpg' % args.img_size)
    draw_act(img, y, activation_pixels, img_act_file, args.img_size)
    draw_quad(img, quad_scores, quad_after_nms, img_quad_file, args.img_size)
